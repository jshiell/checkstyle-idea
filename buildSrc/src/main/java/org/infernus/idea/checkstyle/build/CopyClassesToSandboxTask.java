    ...
    extends Copy
{
    ...
    private void configureTask(final boolean pIsTest) {
        setDescription("Copy classes from \'" + CustomSourceSetCreator.CSACCESS_SOURCESET_NAME + "\' sourceset into "
                + "the prepared " + (pIsTest ? "test " : "") + "sandbox");
        into(new File(getProject().getBuildDir(), "idea-sandbox/plugins" + (pIsTest ? "-test" : "") + "/CheckStyle"
                + "-IDEA/" + TARGET_SUBFOLDER));
    }
