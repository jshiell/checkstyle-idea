...
public class MavenCheckstyleConfigurator implements MavenAfterImportConfigurator {
    ...
    private static MavenId findCheckstyleMavenId(@NotNull final MavenPlugin checkstyleMavenPlugin,
        ...
        return checkstyleMavenPlugin.getDependencies().stream().filter(
            ...
            return findCheckstyleMavenIdInPom(mavenPluginPath, project);
        });
    }

    @NotNull
    private static MavenId findCheckstyleMavenIdInPom(@NotNull final Path pomPath,
        @NotNull final Project project) {
        final var mavenPluginVirtualFile = Objects.requireNonNull(
            VirtualFileManager.getInstance().findFileByNioPath(pomPath));

        return ReadAction.compute(() -> {
            final var mavenDomProjectModel = Objects.requireNonNull(
                MavenDomUtil.getMavenDomProjectModel(project, mavenPluginVirtualFile));
    ...
    private static MavenProject findMavenProject(
        ...
        // Checkstyle Plugin will is what will be used to load settings for the project.
